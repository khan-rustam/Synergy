...
const PageTransition: React.FC<PageTransitionProps> = ({ children, className = '' }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setIsLoading(false);
    }, 1200); // Slightly longer loading time to appreciate the animation
  ...
  return (
    <AnimatePresence mode="wait">
      {isLoading ? (
        <motion.div
          key="loader"
          className="fixed inset-0 flex items-center justify-center bg-synergy-dark z-50 overflow-hidden"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
